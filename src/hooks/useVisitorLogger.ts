  ...
  useEffect(() => {
    const logVisitor = async () => {
      try {
        ...
        // 2. 기기 및 브라우저 정보 가져오기 (react-device-detect 활용)
        const deviceType = isMobile ? 'Mobile' : 'PC';
        const deviceInfo = `${deviceType} (${osName} / ${browserName})`;

        // 3. 유입 경로(이전 페이지 주소) 가져오기
        const referrer = document.referrer || 'Direct (주소창 직접 입력)';
        ...
        formData.append('entry.1523286216', referrer);    // 5. REFERRER 칸
        ...
        console.log('방문자 로그(유입 경로 포함)가 구글 시트에 기록되었습니다.');
      } catch (error) {
